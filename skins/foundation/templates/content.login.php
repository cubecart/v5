...
<form action="{$VAL_SELF}" id="login_form" method="post">
	<p class="show-for-small-only">{$LANG.account.want_to_signup} <a href="?_a=register">{$LANG.account.register_here}</a></p>
	<h2>{$LANG.account.login}</h2>
		...
		<div class="row">
			<div class="small-12 columns"><input type="checkbox" name="remember" id="login-remember" value="1" class="" {if $REMEMBER}checked{/if} /><label for="login-remember">{$LANG.account.remember_me}</label></div>
		</div>
		<div class="row">
