<form action="{$VAL_SELF}" method="post" enctype="multipart/form-data">
   ...
   {if isset($CTRL_FORM)}
   <div class="row">
      <div class="small-12 large-8 columns"><label for="addr_description">{$LANG.common.description}</label><input type="text" name="description" id="addr_description" {if !empty($DATA.description)}value="{$DATA.description}"{else}value="{$LANG.address.billing_address}" onclick="this.value=''"{/if} required placeholder="{$LANG.common.description} {$LANG.form.required}"></div>
   </div>
   <div class="row">
