<form id="gc_form" action="{$VAL_SELF}" method="post">
  ...
  <p>{$LANG_CERT_VALUES}</p>
	...
	{/if}
	<div class="row"><div class="small-12 large-8 columns"><label for="gc-message">{$LANG.common.message} {$LANG.common.optional}</label><span><textarea name="gc[message]" id="gc-message" class="textbox">{$POST.message}</textarea></div></div>
