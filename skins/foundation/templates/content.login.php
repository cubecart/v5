<form action="{$VAL_SELF}" id="login_form" method="post">
	<h2>{$LANG.account.login}</h2>
		...
		<div class="row">
			<div class="small-8 columns"><input type="checkbox" name="remember" id="login-remember" value="1" class="" {if $REMEMBER}checked="checked" {/if} /><label for="login-remember">{$LANG.account.remember_me}</label></div>
		</div>
		<div class="row">
