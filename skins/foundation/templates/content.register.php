...
<form action="{$VAL_SELF}" method="post" name="registration">
	...
	{if $TERMS_CONDITIONS}
	<div class="row">
		<div class="large-7 columns"><label for="register-terms">{$LANG.account.register_terms_agree}<a href="{$TERMS_CONDITIONS}" target="_blank"> &raquo;</a></label></div>
		<div class="large-1 columns left text-right"><input type="checkbox" id="register-terms" name="terms_agree" value="1" {$TERMS_CONDITIONS_CHECKED} /></div>
	</div>
	{/if}
	<div class="row">
		<div class="large-7 columns"><label for="register-mailing">{$LANG.account.register_mailing}</label></div>
		<div class="large-1 columns left text-right"><input type="checkbox" id="register-mailing" name="mailing_list" value="1" {if isset($DATA.mailing_list) && $DATA.mailing_list == 1}checked="checked"{/if} /></div>
	</div>
	<div class="row">
